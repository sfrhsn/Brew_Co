...
async function loadUserData() {
    try {
        ...
        console.log('✅ User data loaded:', window.currentUser?.username || 'guest');
    } catch (error) {
...
async function loadMenuFromDatabase() {
    try {
        ...
        const response = await fetch('handlers/get_menu.php');
        const data = await response.json();
        ...
        if (data.success && data.menus && data.menus.length > 0) {
            ...
            data.menus.forEach(menu => {
                const descriptionLines = menu.description.split('\n').filter(line => line.trim());
                
                window.menuDetailsFromDB[menu.menu_key] = {
                    title: menu.title,
                    image: menu.image,
                    price: parseInt(menu.price),
                    info: descriptionLines.length > 0 ? descriptionLines : [menu.description]
                };
            });
            
            console.log('✅ Menu loaded from DB:', Object.keys(window.menuDetailsFromDB).length, 'items');
            console.log('📋 Menu keys:', Object.keys(window.menuDetailsFromDB));
            
            // Merge DB menu with static menu (DB takes priority)
            window.menuDetails = { ...window.menuDetails, ...window.menuDetailsFromDB };
            
            console.log('📋 Final menu details:', window.menuDetails);
        } else {
            console.warn('⚠️ No menu from database, using static menu');
        }
    } catch (error) {
        console.error('❌ Error loading menu:', error);
        console.log('⚠️ Using static menu as fallback');
    }
}
...
// MODAL & MENU FUNCTIONS - FIXED

window.openModal = function(menuKey) {
    console.log('🔍 Opening modal for:', menuKey);
    console.log('📋 Available menu details:', Object.keys(window.menuDetails));
    
    const item = window.menuDetails[menuKey];
    
    if (!item) {
        console.error('❌ Menu not found:', menuKey);
        console.log('Available keys:', Object.keys(window.menuDetails));
        alert('Menu tidak ditemukan! Menu key: ' + menuKey);
        return;
    }
    ...
    if (!modal || !modalImg || !modalTitle || !modalInfo) {
...
window.openModalDynamic = function(menuKey) {
    console.log('🔍 Opening dynamic modal for:', menuKey);
    window.openModal(menuKey);
};
...
window.addToCart = async function(menuKey, event) {
    ...
    console.log('🛒 Adding to cart:', menuKey);
    ...
    if (!item) {
        console.error('❌ Item not found:', menuKey);
        console.log('Available menu keys:', Object.keys(window.menuDetails));
        alert('Menu tidak ditemukan! Key: ' + menuKey);
        return;
    }
...
window.addToCartDynamic = function(menuKey, event) {
    console.log('🛒 Dynamic add to cart:', menuKey);
    window.addToCart(menuKey, event);
};
...
document.addEventListener('DOMContentLoaded', async function() {
    console.log('🚀 Initializing Brew & Co...');
    ...
    console.log('📊 Available menus:', Object.keys(window.menuDetails).length);
    console.log('👤 Current user:', window.currentUser?.username || 'guest');
});
